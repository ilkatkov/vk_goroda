...
keyboard = {"one_time": False, "buttons": [[get_button(label="Таблица рекордов", color="positive")], [
    get_button(label="Начать заново", color="negative"), get_button(label="Правила", color="primary")]]}
...
def computer_answer(id, word):
    try:
        ...
        step_query = "SELECT words FROM steps WHERE id = {0}".format(
            str(id))  # собираем список названных городов
        ...
        def rnd_city():
            try:
                ...
                vk.method("messages.send", {"peer_id": admin, "message": str(
...
def answer(id, word):
    ...
    info_query = "SELECT EXISTS(SELECT * FROM users WHERE id = {0}".format(
        str(id))+")"
    ...
    if result[0][0] == 0:
        ...
        step_query = "INSERT INTO users VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}')".format(
            str(id), name, surname, "0", last_letter, "")
        cursor.execute(step_query)
        result = cursor.fetchall()
        step_query = "INSERT INTO steps VALUES ('{0}', '{1}')".format(
            str(id), computer_word.title())
    ...
    # игрок зареган и ходит
        ...
        step_query = "SELECT last_letter FROM users WHERE id = {0}".format(
            str(id))  # смотрим последнюю букву
        cursor.execute(step_query)
        result = cursor.fetchall()
        ...
        step_query = "SELECT points FROM users WHERE id = {0}".format(
            str(id))  # узнаем сколько было баллов
        cursor.execute(step_query)
        result = cursor.fetchall()
        ...
        if (computer_word[-1] == "ь") or (computer_word[-1] == "ъ") or (computer_word[-1] == "ы") or (computer_word[-1] == "й"):
            ...
            last_letter = computer_word[-1]
        step_query = "UPDATE users SET points = '{0}', last_letter = '{1}', last_word = '{2}' WHERE id = '{3}'".format(
            str(points+1), last_letter, word, str(id))
...
def records(id):
    try:
        ...
        result = cursor.fetchall()
        if len(result) < 5:
            return vk.method("messages.send", {"peer_id": id, "message": "Зови друзей, чтобы ставить рекорды!", "keyboard": keyboard, "random_id": random.randint(1, 2147483647)})
        else:
            user_query = "SELECT name, surname, points FROM users WHERE id = {0}".format(
                str(id))
            cursor.execute(user_query)
            result_user = cursor.fetchall()[0]
            info = "Таблица рекордов:\n\n1. {0} {1} - {2}\n2. {3} {4} - {5}\n3. {6} {7} - {8}\n4. {9} {10} - {11}\n5. {12} {13} - {14}\n\nВы на {15} месте.\nВаш счёт: {16}".format(
                result[0][0], result[0][1], result[0][2], result[1][0], result[1][1], result[1][
                    2], result[2][0], result[2][1], result[2][2], result[3][0], result[3][1], result[3][2],
                result[4][0], result[4][1], result[4][2], str(
                    int(result.index(result_user))+1), result_user[2]
            )
            return vk.method("messages.send", {"peer_id": id, "message": info, "keyboard": keyboard, "random_id": random.randint(1, 2147483647)})
    except Exception as ex:
        vk.method("messages.send", {"peer_id": admin, "message": str(
            ex), "keyboard": keyboard,  "random_id": random.randint(1, 2147483647)})


def run():
    # ---ОСНОВНОЙ ЦИКЛ ИГРЫ---#
    print("Bot Goroda: run!")
    print("Admin:", admin)
    while True:
        try:
            ...
            if messages["count"] >= 1:
                ...
                if user_word == "начать заново":
                    ...
                    vk.method("messages.send", {
                              "peer_id": id, "message": info, "keyboard": keyboard, "random_id": random.randint(1, 2147483647)})
                else:
                    answer(id, user_word)
        except IndexError:
            vk.method("messages.send", {"peer_id": id, "message": "Я не знаю такого города, либо его не существует!",
                      "keyboard": keyboard, "random_id": random.randint(1, 2147483647)})
        except Exception as ex:
            vk.method("messages.send", {"peer_id": admin, "message": str(
                ex), "keyboard": keyboard,  "random_id": random.randint(1, 2147483647)})
